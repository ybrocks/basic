...
//        관레적으로 bearer라는 문자열을 토큰에 붙여서 전송
            String bearerToken = req.getHeader("Authorization");
            System.out.println(bearerToken);
            if (bearerToken == null){
//            token이 없는경우 검증을 할수가 없으므로 filter chain으로 return
...
//        claims(페이로드)를 기반으로 Authentication객체 생성
//        권한의 경우 다수의 권한을 가질수 있으므로 일반적으로 List로 설계
            List<GrantedAuthority> authorities = new ArrayList<>();
//        권한을 세팅할때 권한은 ROLE_라는 키워드를 붙임으로서 추후 권한체크 어노테이션 사용 가능
            authorities.add(new SimpleGrantedAuthority("ROLE_" + claims.get("role"))); //(claims.get("role").toString())
